...
def get_all_links():
    page = requests.get('https://marvel.com/characters')
...
def create_characters_df():
    ...
    for link in links:
        ...
        marvel_characters['Name'] = soup.find("h1")
        text.replace("\n","").strip()
        marvel_characters['Link'] = link
        print(soup.find('h1').text.replace("\n","").strip(), base_url+ str(link))

        label = soup.findAll('p', {'class': 'bioheader_label'})
        stat = soup.findAll('p', {'class': 'bioheader_stat'})
    for i in range(len(label)):
        column = label[i].text.title()
        if column not in columns:
            column.append(column)
        try:
            marvel_characters[column] = stat[i].text.replace("/n", "").strip()
        except:
